...
# Light effect durations in milliseconds
BLUE_FLASH_DURATION = 500
BLUE_FADE_DURATION = 2000
RED_FLASH_DURATION = 1000
RED_FADE_DURATION = 2500
    ...
    def get_value(self, time: int):
        ...
        if event.type in LIGHT_EVENTS:
            ...
            # Blue flash
                ...
                color = (0, 0, max(0, 1.0 - (time - event.time) / BLUE_FLASH_DURATION), 1)
            # Blue Fade (3s)
            elif event.value == 3:
                color = (0, 0, max(0, 1.0 - (time - event.time) / BLUE_FADE_DURATION), 1)
            # Red lights on
            elif event.value == 5:
                color = (1, 0, 0, 1)
            # Red flash
            elif event.value == 6:
                color = (max(0, 1.0 - (time - event.time) / RED_FLASH_DURATION), 0, 0, 1)
            # Red fade (3s)
            elif event.value == 7:
                color = (max(0, 1.0 - (time - event.time) / RED_FADE_DURATION), 0, 0, 1)
            ...
            if index < len(self.events) - 1:
                ...
                if duration > 0:
                    value = (time - event.time) / duration
                else:
                    value = 0
