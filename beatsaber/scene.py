...
# Saber colors (matching Beat Saber)
LEFT_SABER_COLOR = (1.0, 0.0, 0.0, 1.0)   # Red
RIGHT_SABER_COLOR = (0.0, 0.5, 1.0, 1.0)  # Blue
...
class BSScene:
    ...
    def __init__(self, scene, camera, track):
        ...
        # Saber rendering
        self.saber_prog = resources.programs.load(ProgramDescription(path='programs/saber.glsl'))
        self.saber_vao = self._create_saber_geometry()
    ...
    def resize(self):
        ...
        self.saber_prog['m_proj'].write(self.camera.projection.matrix)

    def _create_saber_geometry(self):
        """Create a simple cylinder geometry for the saber."""
        # Create a simple cylinder (16 segments)
        segments = 16
        vertices = []

        # Bottom center
        for i in range(segments):
            # Triangle from bottom center to edge
            angle1 = 2 * math.pi * i / segments
            angle2 = 2 * math.pi * (i + 1) / segments

            # Bottom face
            vertices.extend([0, 0, 0])  # center
            vertices.extend([math.cos(angle1), 0, math.sin(angle1)])
            vertices.extend([math.cos(angle2), 0, math.sin(angle2)])

            # Top face
            vertices.extend([0, 1, 0])  # center
            vertices.extend([math.cos(angle2), 1, math.sin(angle2)])
            vertices.extend([math.cos(angle1), 1, math.sin(angle1)])
        ...
        vbo = self.scene.ctx.buffer(vertices.tobytes())
        vao = self.scene.ctx.simple_vertex_array(self.saber_prog, vbo, 'in_position')
        return vao

    def render_sabers(self, camera, left_hand=None, right_hand=None):
        """Render the sabers at the given hand positions."""
        cam = camera.matrix
        translate = matrix44.create_from_translation((0, -2, -10), dtype='f4')
        cam = matrix44.multiply(translate, cam)

        self.saber_prog['m_cam'].write(cam)
        self.saber_prog['length'] = 2.0  # Saber length

        if left_hand is not None:
            self.saber_prog['position'] = left_hand.position
            self.saber_prog['rotation'] = left_hand.rotation + math.pi / 2  # Rotate to point forward
            self.saber_prog['color'] = LEFT_SABER_COLOR
            self.saber_vao.render()

        if right_hand is not None:
            self.saber_prog['position'] = right_hand.position
            self.saber_prog['rotation'] = right_hand.rotation + math.pi / 2
            self.saber_prog['color'] = RIGHT_SABER_COLOR
            self.saber_vao.render()
